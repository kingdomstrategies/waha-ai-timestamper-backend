...
def load_model_dict():
    ...
    spinner = halo.Halo(text="Downloading model...", spinner="dots").start()
    if os.path.exists(model_path_name):
        spinner.info("Model path already exists. Skipping downloading.")
    else:
    ...
    spinner = halo.Halo(text="Downloading dictionary...", spinner="dots").start()
    dict_path_name = "/tmp/ctc_alignment_mling_uroman_model.dict"
    if os.path.exists(dict_path_name):
        spinner.info("Dictionary path already exists. Skipping downloading.")
    else:
        torch.hub.download_url_to_file(
    ...
    dictionary = {}
    with open(dict_path_name, encoding="utf-8") as f:
